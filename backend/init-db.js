...
export async function initializeDatabase() {
  console.log('🔄 Checking database connection and initializing tables...');
  let retries = 10;
  while (retries > 0) {
    try {
      // Test query
      ...
      console.log(`⏳ Database connection failed (${err.message}). Retrying in 2 seconds... (${retries} retries left)`);
      retries -= 1;
  ...
  try {
    ...
    // Check if products table is empty and seed if so
    const prodCountRes = await query('SELECT COUNT(*) FROM products');
    if (parseInt(prodCountRes.rows[0].count, 10) === 0) {
      console.log('🌱 Seeding initial products dataset...');
      await query(`
    ...
    // Check if services table is empty and seed if so
    const servCountRes = await query('SELECT COUNT(*) FROM services');
    if (parseInt(servCountRes.rows[0].count, 10) === 0) {
      console.log('🌱 Seeding initial software services dataset...');
      await query(`
